...
private function getIva($idIva,$base,$importe)
{
	...
	public function _getTextoComprobate($id)
	{
		...
		$params['letra']=$model->talonario->letraTalonario;
		...
		return Yii::app()->controller->renderPartial('impresion',array('texto'=>$texto),true);
	}
	...
	{
		$cad='<table width="100%"  border="1" cellpadding="0" cellspacing="0">';
...
<th colspan="3" rowspan="1" style="vertical-align: top; text-align: center;">Cochera<br></th>
<th colspan="3" rowspan="1" style="vertical-align: top; text-align: center;">Local<br></th>
...
<tr>
<th style="vertical-align: top; text-align: center;">Ord.<br>
</th>
<th style="vertical-align: top; text-align: center;">Ext.<br>
</th>
<th style="vertical-align: top; text-align: center;">Esp.<br>
</th>
<th style="vertical-align: top; text-align: center;">Ord.<br>
</th>
<th style="vertical-align: top; text-align: center;">Ext.<br>
...
';
		...
		foreach($model->items as $item){
			...
			$tot+=$total;
			$cad.='<tr>
<td style="vertical-align: top; text-align: center;padding-left:5px;padding-right:5px"><small>'.$item->paraCobrar->propiedad->nombrePropiedad.'</small></td>
<td style="vertical-align: top; padding-left:5px;padding-right:5px"><small>'.$item->detalle.'</small></td>

<td style="vertical-align: top; text-align: center;"><small> '.number_format($item->paraCobrar->getValor(1,1),2).'</small></td>
<td style="vertical-align: top; text-align: center;"><small> '.number_format($item->paraCobrar->getValor(2,1),2).'</small></td>
<td style="vertical-align: top; text-align: center;"><small> '.number_format($item->paraCobrar->getValor(4,1),2).'</small></td>
<td style="vertical-align: top; text-align: center;"><small> '.number_format($item->paraCobrar->getValor(1,2),2).'</small></td>
<td style="vertical-align: top; text-align: center;"><small> '.number_format($item->paraCobrar->getValor(2,2),2).'</small></td>
<td style="vertical-align: top; text-align: center;"><small> '.number_format($item->paraCobrar->getValor(4,2),2).'</small></td>
<td style="vertical-align: top; text-align: center;"><small> '.number_format($item->paraCobrar->getValor(1,3),2).'</small></td>
<td style="vertical-align: top; text-align: center;"><small> '.number_format($item->paraCobrar->getValor(2,3),2).'</small></td>
<td style="vertical-align: top; text-align: center;"><small> '.number_format($item->paraCobrar->getValor(4,3),2).'</small></td>
...
<td style="vertical-align: top; text-align: center;"><small> '.number_format($total,2).'</small></td>
</tr>';
...
public function enviarComprobante($id,$email=null){
	//$dest es por si se quiere enviar a otro mail que no sea el de la entidad
		...
		$mPDF1->WriteHTML($this->_getTextoComprobate($id));
...
public function getNombre()
{
	...
	public function ingresarComprobante($idEntidad,$importe,$fecha,$items,$interes,$bonificacion,$estado,$idTalonario,$idTipoComprobante,$nroComprobante,$importeFavor)
	{
		$id=null;
		$resElec=null;
		if(Talonarios::model()->findByPk($idTalonario)->esElectronico){
	...
	{
	    $tal=Talonarios::model()->findByPk($this->idTalonario);
	    if($tal!=null)
	    $tal->incrementaProximo();
	    parent::afterSave();
	}
