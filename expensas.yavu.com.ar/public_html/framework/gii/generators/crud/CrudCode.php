...
class CrudCode extends CCodeModel
{
	...
	public function generateActiveLabel($modelClass,$column,$class=null)
	{
		return "\$form->labelEx(\$model,'{$column->name}',array('class'=>'$class'))";
	}

	public function generateActiveField($modelClass,$column,$class=null)
	{
		if($column->type==='boolean')
		...
		else
		{
			...
			else
			{
				if(($size=$maxLength=$column->size)>60)
					$size=60;
				return "\$form->{$inputField}(\$model,'{$column->name}',array('class'=>'$class','size'=>$size,'maxlength'=>$maxLength))";
			}
		}
	}
