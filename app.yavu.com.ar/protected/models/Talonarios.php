...
class Talonarios extends CActiveRecord
{
	...
	public function getProx($id)
	{
		$model=TalonariosTipos::model()->findByPk($id);
		return $model->proximo+1;
	}
	public function incrementaProx($id)
	{
		$model=TalonariosTipos::model()->findByPk($id);
		$model->proximo+=1;
		$model->save();
	...
	public static function getPredeterminado()
	{
		...
		foreach($res as $item)
			if($item->esPredeterminado) return $item;
		return null;
	}
	...
	public function rules()
	{
		...
		return array(
			...
			array('proximo,nombreTalonario', 'required'),
