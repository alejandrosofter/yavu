...
class ParaCobrar extends CActiveRecord
{
	...
	private function cargarItems_($idLiquidacion,$propiedad,$idParaCobrar,$cargaCochera=false)
	{
		$importe=0;
		$coef=0;
		$coeficienteAplica=0;
		foreach(GastosTipos::model()->findAll() as $tipoGasto){
				if($tipoGasto->id==4){//ES GASTO ESPEFICIFO
					...
					$coef=($cargaCochera?$propiedad->porcentajeCochera:$propiedad->porcentaje)/10;
					$coeficienteAplica=($cargaCochera?$propiedad->porcentajeCochera:$propiedad->porcentaje);
				}
				
				$importe=$importe*$coef;
				$model=new ParaCobrarItems;
				$model->idParaCobrar=$idParaCobrar;
				$model->importe=round($importe,Settings::model()->getValorSistema('REDONDEO_IMPORTES')*1,PHP_ROUND_HALF_UP);
	...
	public function ingresarItems($idLiquidacion)
	{
		$this->cargarItems_($idLiquidacion,$this->propiedad,$this->id);
		if($this->propiedad->tieneCochera)$this->cargarItems_($idLiquidacion,$this->propiedad,$this->id,true);
		$this->cargarFondoReserva($idLiquidacion,$this->id);
	...
	private function cargarFondoReserva($idLiquidacion,$idParaCobrar)
	{
		$liq=Liquidaciones::model()->findByPk($idLiquidacion);
		$coef=(100/count($liq->edificio->propiedadesActivas))/100;
		$importe=$liq->importeFondoReserva*$coef;
		if($liq->importeFondoReserva>0){
				$model=new ParaCobrarItems;
				$model->idParaCobrar=$idParaCobrar;
				$model->importe=round($importe,Settings::model()->getValorSistema('REDONDEO_IMPORTES')*1,PHP_ROUND_HALF_UP);
