...
define ("TRA", dirname(__FILE__)."/resources/".Yii::app()->user->usuario."/TRA.xml");
define ("TA", dirname(__FILE__)."/resources/".Yii::app()->user->usuario."/TA.xml");               # Ticket de Acceso, from WSAA
define ("TRA_TMP", dirname(__FILE__)."/resources/".Yii::app()->user->usuario."/TRA.tmp");
...
define ("ICON_ERROR",'<img src="images/iconos/glyphicons/glyphicons_197_remove.png" > ');
...
class TicketAcceso
{
      ...
      public function __construct($cert,$pk)
      {
          ...
          if (!file_exists($cert)) {exit(ICON_ERROR." ERROR: Por Favor genere el PEDIDO! ".$cert);}
          if (!file_exists($pk)) {exit(ICON_ERROR." ERROR: Por Favor genere el PEDIDO! ");}
          if (!file_exists(WSDL)) {exit(ICON_ERROR." ERROR: No se encuentra el servicio ");}

          $SERVICE='wsfe';

          $cache_life = 60*60*2; //caching time, in seconds
          $ahora=strtotime(Date('Y-m-d H:i:s'));
          $filemtime = @filemtime(TA);
          $dife=$ahora-$filemtime;
           if($dife>1000){
              $this->CreateTRA($SERVICE);
              $CMS=$this->SignTRA($cert,$pk);
              $TA=$this->CallWSAA($CMS);
              if (!file_put_contents(TA, $TA)) {exit();}
          }
      ...
      private function CallWSAA($CMS)
      {
        ...
        file_put_contents(dirname(__FILE__)."/resources/".Yii::app()->user->usuario."/request-loginCms.xml",$client->__getLastRequest());
        file_put_contents(dirname(__FILE__)."/resources/".Yii::app()->user->usuario."/response-loginCms.xml",$client->__getLastResponse());
