...
class LiquidacionesController extends Controller
{
		...
		public function actionMails()
	{
		...
		public function actionImprimirGastos($id,$retorna=false)
		{
			...
			$this->render('impresionGastos',array('model'=>$model,'data'=>$data),$retorna);
		}
		private function getGrillaGastos($liq,$idTipoGasto)
		{
			$sum=0;
			$cad='<table class="table table-condensed2">';
			$cad.='<tr><th style="width:280px">Proveedor</th><th >Concepto</th><th >$ Loc.</th><th >$ Dpto.</th><th >$ Coch.</th><th style="text-align:right">Importe</th></tr>';
	...
	public function actionImprimir($id,$retorna=false)
	{
		...
		$this->render('impresion',array('model'=>$model,'data'=>$data),$retorna);
	}
	...
	{
		$cad='<table class="table table-condensed2">';
		$cad.='<tr><th>Un.Func</th><th>Inquilino</th><th>% Depto.</th><th>% Coch.</th><th>Ord.Depto</th><th>Ext.Depto</th><th class="ordCochera">Ord.Coch</th><th class="extCochera">Ext.Coch</th><th>Esp.</th>';
	...
	private function getRow($item,$campo,$num=null)
	{
		...
		{
			$arr=Comprobantes::model()->ingresarComprobante($_GET['idEntidad'],$_GET['importe'],$_GET['fecha'],$_GET['items'],$_GET['interesDescuentos'],$_GET['estado'],$_GET['idTalonario'],$_GET['idTipoComprobante'],$_GET['nroComprobante'],$_GET['credito']);
			if($arr['id']!=null){
				ParaCobrar::model()->modificarItems($_GET['items']);
				if(!isset($_GET['creditos']))Entidades::model()->ingresarCredito($_GET['idEntidad'],$_GET['credito']);
